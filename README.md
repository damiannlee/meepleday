...
- **구현은 M0(재설계 기반) 착수 전** — 아래 "구현 현황"은 재정의 이전에 만들어진 것이라 일부는 제거·교체 대상.
...
| 영역 | 상태 | 비고 |
|---|---|---|
| 이벤트 피드 (필터·정렬·페이징) | 구현됨 | **카드 그리드 → 타임라인으로 교체 예정** ([PRD §5.1](docs/prd.md#51-첫-화면--타임라인-기간-그룹)) |
| 이벤트 상세 | 구현됨 | 달성률 바는 제거 대상 ([PRD §4.4](docs/prd.md#44-데이터-신선도)) |
...
| Game 엔티티 | 미구현 | M0 ([PRD §4.2](docs/prd.md#42-game-엔티티)) |
| 오프라인 행사 유형 | 미구현 | `EventType`에 `OFFLINE_EVENT` 추가 필요 |
